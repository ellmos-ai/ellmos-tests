...
def analyze_system(root_path: str) -> dict:
    ...
    # Score berechnen (fuer Runner-Kompatibilitaet)
    doc_ratio = stats["metrics"]["doc_ratio"]
    files_per_dir = stats["metrics"]["files_per_dir"]
    score = 3.0
    if doc_ratio >= 15:
        score += 1.0
    elif doc_ratio >= 5:
        score += 0.5
    if 3 <= files_per_dir <= 15:
        score += 1.0
    elif files_per_dir <= 30:
        score += 0.5
    stats["score"] = min(5.0, max(1.0, round(score, 2)))
